...
namespace FaultLocalization
{
    ...
    public partial class FLDBServer
    {
        ...
        public static List<int>[] ReadTestCaseChangeClassInfo(int ID, string description, int itimes)
        {
            ...
            //读取缺陷版本
            string strSQL = "SELECT 选取用例总数, 选取成例数, 选取失例数, 成例选取记录, 失例选取记录 FROM 随机变更用例类别表"
        ...
        /// <returns>result[0]:suc result[1]:fal</returns>
        public static List<int[]>[] ReadTestCaseChangeClassDivInfo(int ID, FLConfigure cfg, int stimes, int dtimes)
        {
            List<int[]>[] result = null;
            ...
            //读取缺陷版本
            string strSQL = "SELECT 分组数量,成例分组记录,失例分组记录 FROM 随机变更用例类别_集成实验分组表"
                         + " WHERE ID=" + ID.ToString()
                         + " AND 变更用例选取策略描述=" + "'" + cfg.ClassChangeSelectStrategy + "'"
                         + " AND 变更用例实验次数=" + stimes.ToString()
            ...
            result[0] = new List<int[]>();
            result[1] = new List<int[]>();
            string rawsucStr = mDataSet.Tables[0].Rows[0]["成例分组记录"].ToString();
            string rawfalStr = mDataSet.Tables[0].Rows[0]["失例分组记录"].ToString();
            // Gzip解压
        ...
        public static void InsertTestCaseChangeClassInfo(int ID, string description, int itimes, List<int>[] cases)
        {
            List<int>[] temp = ReadTestCaseSelectInfo(ID, description, itimes);
            if (null != temp)
                UpdateTestCasChangeClassInfo(ID, description, itimes, cases);
            else  // 李成龙改
            {
                StringBuilder sucList = new StringBuilder();
                StringBuilder falList = new StringBuilder();

                List<int> sucCaseIDs = cases[0];
                if (0 < sucCaseIDs.Count)
                {
                    sucList.Append(sucCaseIDs[0].ToString());
                    for (int i = 1; i < sucCaseIDs.Count; i++)
                        sucList.Append("," + sucCaseIDs[i].ToString());
                }

                List<int> falCaseIDs = cases[1];
                if (0 < falCaseIDs.Count)
                {
                    falList.Append(falCaseIDs[0].ToString());
                    for (int i = 1; i < falCaseIDs.Count; i++)
                        falList.Append("," + falCaseIDs[i].ToString());
                }

                string sSQLString = "INSERT INTO 随机变更用例类别表(ID,用例选取策略描述, 实验次数, 选取用例总数, 选取成例数, 选取失例数, 成例选取记录, 失例选取记录)" + "VALUES("
                                 + ID.ToString() + ","
                                 + "'" + description + "',"
                                 + itimes.ToString() + ","
                                 + (sucCaseIDs.Count + falCaseIDs.Count).ToString() + ","
                                 + sucCaseIDs.Count.ToString() + ','
                                 + falCaseIDs.Count.ToString() + ','
                                 + "'" + sucList.ToString() + "',"
                                 + "'" + falList.ToString() + "')";
        ...
        public static void InsertTestCaseChangeClassDivInfo(int ID, FLConfigure cfg, int stimes, int dtimes, List<FLRunsGroupInfo> groups)
        {
            List<int[]>[] temp = ReadTestCaseChangeClassDivInfo(ID, cfg, stimes, dtimes);
            if (null != temp)
                UpdateTestCasChangeClassDivInfo(ID, cfg, stimes, dtimes, groups);
            else  // 李成龙改
            {
                ...
                string sSQLString = "INSERT INTO 随机变更用例类别_集成实验分组表(ID, 变更用例选取策略描述, 变更用例实验次数, 分组策略描述, 类别比例, 实验次数, 分组数量, 成例分组记录, 失例分组记录)" + "VALUES("
                     + ID.ToString() + ","
                     + "'" + cfg.ClassChangeSelectStrategy + "',"
                     + stimes.ToString() + ","
        ...
        /// <param name="description">用例选取策略描述</param>
        /// <param name="itimes">实验次数</param>
        public static void DeleTestCaseChangeClassInfo(int ID, string description, int itimes)
        {
            try
            {
                string strSQL = "DELETE FROM 随机变更用例类别表"
                              + " WHERE ID=" + ID.ToString()
                              + " AND 用例选取策略描述=" + "'" + description + "'"
                              + " AND 实验次数=" + itimes.ToString();
        ...
        /// <param name="cfg">实验配置</param>
        /// <param name="itimes">分组实验次数</param>
        public static void DeleTestCaseChangeClassDivinfo(int ID, FLConfigure cfg, int stimes, int dtimes)
        {
            try
            {
                string strSQL = "DELETE FROM 随机变更用例类别_集成实验分组表"
                         + " WHERE ID=" + ID.ToString()
                         + " AND 变更用例选取策略描述=" + "'" + cfg.ClassChangeSelectStrategy + "'"
                         + " AND 变更用例实验次数=" + stimes.ToString()
                         + " AND 分组策略描述=" + "'" + cfg.ClassRatioDivideStrategy + "'"
        ...
        /// <param name="itimes">实验次数</param>
        /// <param name="group">用例组</param>
        private static void UpdateTestCasChangeClassInfo(int ID, string description, int itimes, List<int>[] cases)
        {
            try
            {
                DeleTestCaseChangeClassInfo(ID, description, itimes);
                InsertTestCaseChangeClassInfo(ID, description, itimes, cases);
            }
            catch (Exception e)
            {
        ...
        /// <param name="itimes">分组实验次数</param>
        /// <param name="groups">分组</param>
        private static void UpdateTestCasChangeClassDivInfo(int ID, FLConfigure cfg, int stimes, int dtimes, List<FLRunsGroupInfo> groups)
        {
            try
            {
                DeleTestCaseChangeClassDivinfo(ID, cfg, stimes, dtimes);
                InsertTestCaseChangeClassDivInfo(ID, cfg, stimes, dtimes, groups);
            }
            catch (Exception e)
            {
