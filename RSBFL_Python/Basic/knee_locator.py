...
class KneeLocator(object):
    ...
    def __init__(
        ...
        if interp_method == "interp1d":
            ...
            self.Ds_y = p(x)
        elif interp_method == "convolve":
            windowsize = 2
            window = np.ones(int(windowsize)) / float(windowsize)
            self.Ds_y = np.convolve(self.y, window, 'same')
        else:
            raise ValueError(
        ...
        # global maxima
        self.global_maxima_indice = self.maxima_indices[0]
    ...
    def plot_global_maximal_normalized(self, figsize: Optional[Tuple[int, int]] = None):
        """Plot the normalized curve, the difference curve (x_difference, y_normalized) and the knee, if it exists.
        ...
        plt.vlines(
            self.x_normalized[self.global_maxima_indice],
            plt.ylim()[0],
            plt.ylim()[1],
            linestyles="--",
            label="maximal",
