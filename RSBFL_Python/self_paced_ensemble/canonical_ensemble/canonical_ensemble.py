    ...
    def fit(self, X, y):
        ...
        for ibagging in range(self.n_estimators):
            ...
            train_maj = df_maj.sample(frac=b, replace=True) \
                if len(df_maj.sample(frac=b, replace=True)) != 0 else df_maj.sample(n=1, replace=True)
            train_min = df_min.sample(frac=b, replace=True)\
                if len(df_min.sample(frac=b, replace=True)) != 0 else df_min.sample(n=1, replace=True)
...
class BalanceCascadeClassifier(BaggingClassifier):
    ...
    def fit(self, X, y, print_log=False, visualize=False):
        ...
        df['label'] = y
        df_maj = df[y == 0];
        n_maj = df_maj.shape[0]
        df_min = df[y == 1];
        n_min = df_min.shape[0]
        ...
        for ibagging in range(1, self.n_estimators):
            ...
            if visualize:
                df_train.plot.scatter(x=0, y=1, s=3, c='label', colormap='coolwarm',
                                      title='Iter {} training set'.format(ibagging))
