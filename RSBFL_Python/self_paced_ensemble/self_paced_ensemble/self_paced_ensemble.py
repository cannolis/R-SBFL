...
class SelfPacedEnsembleClassifier(BaseEnsemble, ClassifierMixin):
    ...
    def _random_under_sampling(self, X_input, y_input, sample_num):
        """Private function used to perform random under-sampling."""
        np.random.seed(self.random_state)
        idx = np.random.choice(len(X_input), sample_num, replace=False)
        # x_index = np.arange(len(X_input))
        X_output = X_input[idx]
        y_output = y_input[idx]
        # sampled_index = {str(y_input[0]): x_index[idx]}

        return X_output, y_output

    def _ass_num(self, _len_bins, sample_num, _sample_rate):
        ...
        temp_sample_bins = np.array(
            [min(len_bins[i_bins], temp_sample_bins[i_bins]) for i_bins in range(len(len_bins))])
        if temp_sample_bins.sum() < sample_num:
            ...
            add_sample_bins = self._ass_num(len_bins - temp_sample_bins, sample_num, sample_rate)
            temp_sample_bins = temp_sample_bins + add_sample_bins
        return temp_sample_bins

    def _self_paced_under_sampling_base(self, X_input, y_input, sample_num, i_estimator, y_pred_proba_buffer):
        # Update hardness value estimation
        hardness = self.hardness_func(y_input,
                                      y_pred_proba_buffer[:, self.class_index_min])  # 以 lable为"1"的列 作为标的，相减刚好是hardness

        # If hardness values are not distinguishable, perform random smapling
        if hardness.max() == hardness.min():
            X_output, y_output = self._random_under_sampling(X_input, y_input, sample_num)
        ...
        # Else allocate majority samples into k hardness bins
            ...
            bins = []
            ave_contributions = []
            X_input_indexs = np.arange(len(X_input))
            bins_indexs = []
            for f_bins in range(self.k_bins):
                ...
                if f_bins == (self.k_bins - 1):
                    idx = idx | (hardness == hardness.max())
                bins.append(X_input[idx])
                bins_indexs.append(X_input_indexs[idx])
            ...
            # TODO:Modified
            # n_sample_bins = self._ass_num(len_bins, sample_num, sample_rate)
            n_sample_bins = self._ass_num(len_bins, sample_num,
                                          weights)  # n_sample_bins = n_sample_bins.astype(int) + 1
            ...
            if len(sampled_bins) == 1:
                X_output = sampled_bins[0]
            else:
    ...
    def _self_paced_under_sampling(self,
        ...
        X_train_min, y_train_min = self._self_paced_under_sampling_base(X_min, y_min, min_sample_num,
                                                                        i_estimator, self.y_min_pred_proba_buffer)
    ...
    def update_min_pred_buffer(self, X_min):
        """Maintain a latest prediction probabilities of the minority
           training data during ensemble training."""

        if self.m_buffered_estimators_ > len(self.estimators_):
            raise ValueError(
                'Number of buffered estimators ({}) > total estimators ({}), check usage!'.format(
                    self.m_buffered_estimators_, len(self.estimators_)))
        if self.m_buffered_estimators_ == 0:
        ...
        for i in range(self.m_buffered_estimators_, len(self.estimators_)):
    ...
    def fit(self, X, y, label_maj=None, label_min=None):
        ...
        for i_iter in iterations:
            ...
            # update current minority training data prediction
            self.update_min_pred_buffer(self.X_min)
...
def parallel_predict_proba_wrapper(args):
    return _parallel_predict_proba(*args)